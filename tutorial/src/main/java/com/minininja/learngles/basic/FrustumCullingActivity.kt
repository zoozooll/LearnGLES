...
class FrustumCullingActivity : GLActivity() {
    ...
    override fun createTouchCallback(): Layer3DTouchCallback {
        ...
        return object : Layer3DTouchCallback {
            override fun onSingleTouch(
                ...
                if (!active) return
                Log.d("FrustumCullingActivity", "onSingleTouch: $prevPoint, $point")
            ...
            override fun onDoubleTouch(
                ...
                if (!active) return
                Log.d("FrustumCullingActivity", "onDoubleTouch: $prevPoint -> $point, $prevPoint1 -> $point1")
                val event = mapOf("event_id" to "target_camera_touching_event",
            ...
            override fun onDoubleClick(event: Offset) {
                ...
                val event = mapOf("event_id" to "target_camera_touching_event",
                    "reset" to 1)
                NativeHelper.sendCommands(event)
                Log.d("FrustumCullingActivity", "Custom DoubleClick at: $event")
                glSurfaceView?.requestRender()
            }
