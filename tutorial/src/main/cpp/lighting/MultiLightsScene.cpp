#include "MultiLightsScene.h"

#include "Shader.h"
#include "Texture.h"
#include "TargetCamera.h"

MultiLightsScene::MultiLightsScene() {
...
void MultiLightsScene::init() {
    ...
    // ------------------------------------
    m_pLightingShader = new Shader("shaders/multiple_lights/multiple_lights.vert",
            "shaders/multiple_lights/multiple_lights.frag");
    m_pLightCubeShader = new Shader("shaders/multiple_lights/light_cube.vert",
            "shaders/multiple_lights/light_cube.frag");
...
void MultiLightsScene::resize(int width, int height) {
    m_camera->setAspec((float) width / (float) height);
    glViewport(0, 0, width, height);
}

void MultiLightsScene::draw() {
    m_camera->update();
    // render
    // ------
    ...
    if(m_pLightingShader)
    {
        ...
        // spotLight
        m_pLightingShader->setVec3("spotLight.position", m_camera->getPosition());
        m_pLightingShader->setVec3("spotLight.direction", m_camera->getFront());
...
std::map<std::string, std::any> MultiLightsScene::propertyEvent(std::map<std::string, std::any> &map) {
...
void MultiLightsScene::parseTargetCameraEvent(std::map<std::string, std::any> &event) {
