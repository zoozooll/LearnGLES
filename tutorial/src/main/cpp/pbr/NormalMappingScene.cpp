...
void NormalMappingScene::init() {
    ...
    dynamic_cast<TargetCamera*>(m_camera)->setRadius(3.0f);
    ...
    // -------------------------
    m_pShader = new Shader("shaders/normal_mapping/normal_mapping.vert",
            "shaders/normal_mapping/normal_mapping.frag");

    // load textures
    // -------------
    m_diffuseMap = loadTexture("textures/brickwall.jpg");
    m_normalMap  = loadTexture("textures/brickwall_normal.jpg");
...
std::map<std::string, std::any> NormalMappingScene::propertyEvent(std::map<std::string, std::any> &map) {
