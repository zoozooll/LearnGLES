...
//    glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS); //TODO Not support in GLES
    ...
    // ---------------------------------
    hdrTexture = loadTexture("textures/hdr/newport_loft.hdr");
...
void IblScene::draw() {
    ...
    if(pbrShader && backgroundShader)
    {
        ...
        {
            glm::vec3 newPos = lightPositions[i] + glm::vec3(sin(GetEscapeSecs() * 5.0) * 5.0, 0.0, 0.0);
...
std::map<std::string, std::any> IblScene::propertyEvent(std::map<std::string, std::any> &map) {
