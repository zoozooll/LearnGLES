#include "TheoryScene.h"

#include <GLES3/gl32.h>

#include "TargetCamera.h"

TheoryScene::TheoryScene() {

}

void TheoryScene::init() {
    m_camera = new TargetCamera;
}

void TheoryScene::resize(int width, int height) {
    m_camera->setAspec((float) width / (float) height);
    glViewport(0, 0, width, height);
}

void TheoryScene::draw() {
    m_camera->update();
    glClearColor(0.2f, 0.3f, 0.3f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void TheoryScene::destroy() {
    delete m_camera;
}
...
std::map<std::string, std::any> TheoryScene::propertyEvent(std::map<std::string, std::any> &map) {
...
void TheoryScene::parseTargetCameraEvent(std::map<std::string, std::any> &event) {
